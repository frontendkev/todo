...
export default function BottomLink({filterTodo, title, count, filter, currentPage, page}) {
    ...
    return (
        <div className="relative w-fit h-full flex flex-row items-center gap-x-1">
            <button onClick={() => filterTodo(filter)} className={`relative capitalize font-quicksandsemibold ${currentPage === page ? 'text-rose-500':'text-gray-300'} text-[0.8em] md:text-[0.8em] transition-all duration-300 ease-in-out`}>
                {
                    title
                }
            </button>
